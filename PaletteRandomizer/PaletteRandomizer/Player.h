...
#define LoadPlayerPalettes()                                                                                                                         \
    color palColors[PLAYER_PALETTE_BANK_COUNT][256];                                                                                                 \
    for (int32 b = 0; b < PLAYER_PALETTE_BANK_COUNT; b++) {                                                                                          \
        for (int32 i = 0; i < 256; i++) {                                                                                                            \
            palColors[b][i] = RSDK.GetPaletteEntry(PLAYER_PALETTE_BANK_START + b, i);                                                                \
        }                                                                                                                                            \
    }                                                                                                                                                \
                                                                                                                                                     \
    for (int32 i = 0; i < PLAYER_PALETTE_BANK_COUNT; i++) {                                                                                          \
        char palName[16];                                                                                                                            \
        sprintf_s(palName, (int32)sizeof(palName), "Players%d.act", i + 1);                                                                          \
        RSDK.LoadPalette(PLAYER_PALETTE_BANK_START + i, palName, 0b0000000000000000);                                                                \
    }

#define RestorePalettes()                                                                                                                            \
    for (int32 b = 0; b < PLAYER_PALETTE_BANK_COUNT; b++) {                                                                                          \
        for (int32 i = 0; i < 256; i++) {                                                                                                            \
